...
from FDataBase_courses import FDataBase

DATABASE = '/tmp/flsk_courses.db'
DEBUG = True
...
app.config.update(dict(DATABASE=os.path.join(app.root_path, 'flsk_courses.db')))
...
def info():
    db = get_db()
    dbase = FDataBase(db)
    return render_template('info.html', menu=dbase.get_menu(), title="Информация",
                           posts=dbase.get_posts_annonce())
